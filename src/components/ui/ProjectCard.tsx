...
export const ProjectCard = ({ project }: ProjectCardProps) => {
    return (
        /* 2. Ubah div terluar menjadi komponen Link */
        <Link 
            href={project.url} 
            target="_blank" // Membuka di tab baru (standar portfolio premium)
        ...
        >
            ...
            <div className="relative w-full aspect-video bg-zinc-900 overflow-hidden rounded-sm border border-white/5 transition-colors duration-500 group-hover:border-accent/30">
                <Image 
                    src={project.thumbnail || "/images/khoas1.png"} 
                    alt={project.title}
                    fill
            ...
            {/* Bagian Informasi Meta (Tetap sama) */}
            <div className="flex flex-col px-1">
                ...
                {project.description && (
                    <div className="mt-4 border-t border-white/5 pt-3">
                        <p className="text-sm font-light text-support/60 leading-relaxed line-clamp-2 break-words group-hover:text-support/90 transition-colors duration-300">
                            {project.description}
                        </p>
                    </div>
                )}
